...
export const UiGenerales = React.forwardRef(
  ({ passOrIne, datos, children }, ref) => {
        ...
        {passOrIne === "ine" && (
          <>
            <p className="texto-justificado" ref={ref}>
              ...
              Instituto Nacional Electoral con Clave Única de Registro de
              Población <strong>{datos.curp.toUpperCase()}</strong> (
              {procesarCURP(datos.curp)}
              ), expedida por la Secretaría de Gobernación, y con Clave Federal
              de Registro de Contribuyentes{" "}
              <strong>{datos.rfc.toUpperCase()}</strong> (
              {procesarCURP(datos.rfc)}), expedida por el Servicio de
